...
static void test_setup_succeeds_with_registered_mac(void) {
  ...
  TEST_ASSERT_FALSE_MESSAGE(result.response.api_key.isEmpty(), "Response should carry a non-empty api_key");
  TEST_ASSERT_FALSE_MESSAGE(result.response.friendly_id.isEmpty(), "Response should carry a non-empty friendly_id");
}
...
static void test_setup_returns_status_error_for_unknown_mac(void) {
  ...
  TEST_ASSERT_EQUAL_MESSAGE(HTTPS_NO_ERR, result.error, "Transport should succeed even when MAC is unregistered");
  ...
  TEST_ASSERT_NOT_EQUAL_MESSAGE(200, result.response.status, "Unregistered MAC should not return 200");
}
