...
// Local helpers (definitions later in this file)
...
// Stub touchbar I2C locks when IQS323 task is not present
...
static void downloadSetupImage()
{
  ...
  withHttp(filename, [&](HTTPClient *https, HttpError error) -> bool
    ...
    {
      counter = ImagePipeline::downloadStream(stream, contentSize, buffer);
    }
#else
    ...
    if (counter == DISPLAY_BMP_IMAGE_SIZE)
    {
      ...
      ImagePipeline::writeImageToFile("/logo.bmp", buffer, DEFAULT_IMAGE_SIZE);
    ...
    {       
      ...
      ImagePipeline::writeImageToFile("/logo.png", buffer, counter);
      free(buffer);
      buffer = nullptr;
