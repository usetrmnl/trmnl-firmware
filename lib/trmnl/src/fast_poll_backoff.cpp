...
uint32_t fastPollSleepSeconds(uint32_t streak) {
  if (streak <= FAST_POLL_SNAPPY_POLLS) return FAST_POLL_SLEEP_SECONDS;
  if (streak <= 60) return 60;
  if (streak <= 70) return 900;
  return 3600;
}
...
void fastPollStreakReset(Persistence &persistence) {
  if (persistence.readUint(FAST_POLL_STREAK_KEY, 0) != 0) persistence.writeUint(FAST_POLL_STREAK_KEY, 0);
}
