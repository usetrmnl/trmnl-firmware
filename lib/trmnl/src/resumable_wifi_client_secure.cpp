...
// a negative error.
static int rwcs_start(sslclient_context *ssl_client, const IPAddress &ip, uint32_t port,
                      const char *hostname, int timeout, const char **alpn_protos, bool try_resume)
{
  int ret;
  int enable = 1;
  ...
  ret = mbedtls_ctr_drbg_seed(&ssl_client->drbg_ctx, mbedtls_entropy_func,
                              &ssl_client->entropy_ctx, (const unsigned char *)rwcs_pers, strlen(rwcs_pers));
  if (ret < 0)
    return RWCS_ERR(ret);

  if ((ret = mbedtls_ssl_config_defaults(&ssl_client->ssl_conf, MBEDTLS_SSL_IS_CLIENT,
...
int ResumableWiFiClientSecure::connect(IPAddress ip, uint16_t port)
{
  return connectResumable(ip, port, nullptr);
}
