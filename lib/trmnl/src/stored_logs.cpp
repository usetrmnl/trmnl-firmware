...
StoredLogs::StoredLogs(uint8_t old_count, uint8_t new_count, const char *log_key, const char *head_key,
                       Persistence &persistence)
    : old_count(old_count), new_count(new_count), log_key(log_key), head_key(head_key), persistence(persistence),
      overwrite_count(0) {}
...
uint32_t StoredLogs::get_overwrite_count() { return overwrite_count; }