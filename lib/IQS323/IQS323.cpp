...
/**
  ...
  */
void IQS323::begin(uint8_t deviceAddressIn, int sdaPinIn, int sclPinIn, uint8_t readyPinIn, bool to_init)
{
...
void IQS323::run(void)
{
  ...
  switch (iqs323_state.state)
  {
    ...
    state machine */
    case IQS323_STATE_NONE:
      Serial.println("IQS323 None state, something seems wrong.");
    break;
...
/**
  ...
  * @brief  Configure GESTURE_SELECT and EVENTS_ENABLE for slide or tap mode.
  *         Slide mode: gesture events only (GESTURE_SELECT=0x0B, EVENTS_ENABLE=0x04).
  *         Tap mode:   no gestures (GESTURE_SELECT=0x00), touch events only (EVENTS_ENABLE=0x02).
  * @param  tap_mode      -> true for tap mode, false for slide mode.
  * @param  stopOrRestart -> Use STOP or RESTART.
  */
