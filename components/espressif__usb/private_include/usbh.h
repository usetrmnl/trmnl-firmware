...
/**
 * @brief Update a device's configuration descriptor in-place (device already configured).
 *
 * Unlike usbh_dev_set_config_desc, this does not require ADDRESS state or enum_lock.
 * Used by CDC-ECM to refresh the descriptor after switching to config 2.
 */
esp_err_t usbh_dev_update_config_desc(usb_device_handle_t dev_hdl, const usb_config_desc_t *config_desc_full);
